class HomeController < ApplicationController
  ...
  def graphs
    ...
    @category_multiseries = category.map {|category| {name: category.name, data: category.expenses.where(:created_at => Time.now.last_week.beginning_of_week..Time.now.end_of_week, :gains => false).group_by_day(:created_at).sum("amount")}}
